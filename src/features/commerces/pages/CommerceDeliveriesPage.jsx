...
function deliveryRowKey(row, index) {
    return String(row.fk_user ?? row.id_delivery ?? `delivery-${index}`);
}
...
function PageHeader({ onNavigateReviews, onNavigateAdd }) {
    return (
        <div
            style={{
                display: "flex",
        ...
        >
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "center" }}>
                <button type="button" onClick={onNavigateReviews} style={secondaryButtonStyle()}>
                    <Star size={18} />
                    Ver reseñas
                </button>
                <button type="button" onClick={onNavigateAdd} style={primaryButtonStyle({ flexShrink: 0 })}>
...
function EmptyState({ onNavigateAdd }) {
    return (
        <div
            style={{
                backgroundColor: "white",
        ...
        >
            ...
            <button type="button" onClick={onNavigateAdd} style={primaryButtonStyle()}>
                <Plus size={18} /> Agregar delivery
            </button>
        </div>
    );
...
function DeliveryRow({ row }) {
    return (
        ...
        >
            <div>
                <p style={{ margin: "0 0 8px 0", fontSize: "16px", fontWeight: "700", color: "#111827" }}>
                    {row.name ?? "—"}
                </p>
...
function DeliveriesList({ rows }) {
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {rows.map((row, index) => (
                <DeliveryRow key={deliveryRowKey(row, index)} row={row} />
            ))}
        </div>
    );
}

function useCommerceStoreSession() {
...
export function CommerceDeliveriesPage() {
    ...
    const { loading, error, rows } = useCommerceStoreSession();

    const goReviews = () => navigate("/comercio/delivery");
    const goAdd = () => navigate("/comercio/deliveries/agregar");
    ...
    return (
        <>
            <PageHeader onNavigateReviews={goReviews} onNavigateAdd={goAdd} />
            {listEmpty ? <EmptyState onNavigateAdd={goAdd} /> : <DeliveriesList rows={rows} />}
        </>
    );
}
