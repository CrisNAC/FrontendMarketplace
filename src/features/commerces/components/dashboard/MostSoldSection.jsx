...
export const MostSoldSection = ({ products = [], loading = false }) => {
    return (
        <div style={{ backgroundColor: "white", borderRadius: "16px", padding: "16px", boxShadow: "0 1px 4px rgba(0,0,0,0.08)", height: "100%" }}>
            ...
            {!loading && products.length === 0 && (
                <p style={{ fontSize: "13px", color: "#6b7280" }}>Aún no hay ventas registradas.</p>
            )}

            {!loading && products.map((product, index) => (
                <div key={product.id} style={{ backgroundColor: "var(--background-white)", borderRadius: "8px", padding: "8px", marginBottom: index < products.length - 1 ? "10px" : 0, display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: "10px" }}>
                    <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "10px" }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: "12px", color: "#6b7280" }}>
                                {formatGuarani(product.price)} · {product.totalSold} vendido{product.totalSold !== 1 ? "s" : ""}
                            </div>
                        </div>
                    </div>
                    <span style={{ backgroundColor: "rgba(22,149,80,0.15)", color: "#16a34a", fontSize: "12px", fontWeight: "600", padding: "2px 10px", borderRadius: "12px", flexShrink: 0 }}>
                        Activo
                    </span>
                </div>
            ))}
